package ltd.matrixstudios.compound.utility.time
...
object TimeUtil {
    ...
    fun getTimeZoneShortName(displayName: String): String {
        val stz = displayName.split(" ".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
    ...
    fun parseTime(time: String): Long {
        ...
        while (matcher.find()) {
            val s = matcher.group()
            val value = s.split("(?<=\\D)(?=\\d)|(?<=\\d)(?=\\D)".toRegex()).dropLastWhile { it.isEmpty() }
                .toTypedArray()[0].toLong()
            var s2: String
            s2 = s.split("(?<=\\D)(?=\\d)|(?<=\\d)(?=\\D)".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()[1]
            val type = s2
            when (s2) {
                "s" -> {
                    totalTime += value
